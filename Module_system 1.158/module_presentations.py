...
##        (gt, ":total_wage", 0),
  ...
  ("multiplayer_duel_start_counter", prsntf_read_only|prsntf_manual_end_only, 0, [
	...
	########### TU ##### chongche ###### 2014.1.9 #######
# ("construct_siege_chongche", 0, mesh_load_window, [
    # (ti_on_presentation_load,
      # [
        # (presentation_set_duration, 999999),
        # (set_fixed_point_multiplier, 1000),
		# (assign, "$num_of_have_portable_ram", 0),
		# (assign, "$construct_siege_total_time", 1),
		# (init_position,pos1),
		# (init_position,pos2),
		
		# (create_game_button_overlay, "$siege_weapon_done", "@Done"),
        # (position_set_x, pos1, 900),
        # (position_set_y, pos1, 25),
        # (overlay_set_position, "$siege_weapon_done", pos1),
		
		# (create_text_overlay, reg0, "@Siege Weapon", tf_center_justify),
		# (position_set_x, pos1, 500),
        # (position_set_y, pos1, 650),
		# (overlay_set_position, reg0, pos1),
		...
		# (create_text_overlay, reg0, "@Ladder num:", tf_center_justify),
		# (overlay_set_position, reg0, pos1),
		
		# (create_number_box_overlay, "$ladder_num", 1, 5),
		# (overlay_set_position, "$ladder_num", pos2),
		# (overlay_set_val, "$ladder_num", 1),
		...
		# (val_mul, reg4, 8),
		# (party_get_slot, ":party_defense", "$current_town", slot_party_defense),#TU 7.20
		# (val_add, ":party_defense", 10),                                         #
		# (val_add, reg4, ":party_defense"), 
		# (assign, "$construct_siege_total_time", reg4),
		# (create_text_overlay, "$total_time_pre", "@Time: {reg4} hours", tf_center_justify),
		# (overlay_set_position, "$total_time_pre", pos3),
		
		# (position_move_y, pos1, -5),
		# (position_move_y, pos2, -5),
		
		# (create_text_overlay, reg0, "@Portable Ram:", tf_center_justify),
		# (overlay_set_position, reg0, pos1),
		
		# (create_check_box_overlay, "$have_portable_ram", "mesh_checkbox_off", "mesh_checkbox_on"),
		# (overlay_set_position, "$have_portable_ram", pos2),
		# (overlay_set_val, "$have_portable_ram", 0),
   # ]),
	# (ti_on_presentation_event_state_change,
     # [
		# (store_trigger_param_1, ":object"),
		...
		# (try_begin),
			# (eq, ":object", "$siege_weapon_done"),
            # (presentation_set_duration, 0),
			
			# (assign, "$g_siege_method", 2),
			# (store_current_hours, ":cur_hours"),
			
			# (assign, reg3, "$num_of_ladder_num"),
			# (val_sub, reg3, 1),
			# (val_mul, reg3, 3),
			# (assign, reg4, "$num_of_have_portable_ram"),
			# (val_mul, reg4, 10),
			# (val_add, reg4, "$construct_siege_total_time"),
			# (val_add, reg4, reg3),
			
			# (store_add, "$g_siege_method_finish_hours",":cur_hours", reg4),
			...
			# (change_screen_return),
		# (else_try),
            # (eq, ":object", "$ladder_num"),
			# (assign, "$num_of_ladder_num", ":value"),
			# (val_sub, ":value", 1),
			# (val_mul, ":value", 3),
			# (assign, reg4, "$num_of_have_portable_ram"),
			# (val_mul, reg4, 10),
			# (val_add, reg4, "$construct_siege_total_time"),
			# (val_add, reg4, ":value"),
			# (overlay_set_text, "$total_time_pre", "@Time: {reg4} hours"),
		# (else_try),
            # (eq, ":object", "$have_portable_ram"),
			# (assign, "$num_of_have_portable_ram", ":value"),
			# (val_mul, ":value", 10),
			# (assign, reg4, "$num_of_ladder_num"),
			# (val_sub, reg4, 1),
			# (val_mul, reg4, 3),
			# (val_add, reg4, "$construct_siege_total_time"),
			# (val_add, reg4, ":value"),
			# (overlay_set_text, "$total_time_pre", "@Time: {reg4} hours"),
		# # (else_try),
			# # (eq, ":object", "$unlimited_reinforcement"),
			# # (try_begin),
				# # (eq, "$if_unlimited_reinforcement", 0),
				# # (val_add, "$if_unlimited_reinforcement", 1),
				# # (overlay_set_alpha, "$reinforcement_stage_wenzi", 0),
				# # (init_position,pos5),
				# # (overlay_get_position, pos5, "$reinforcement_stage"),
				# # (position_move_y, pos5, -100),
				# # (overlay_set_position, "$reinforcement_stage", pos5),
				# # (assign, "$g_reinforcement_stage", 1),
			# # (else_try),
				# # (eq, "$if_unlimited_reinforcement", 1),
				# # (val_add, "$if_unlimited_reinforcement", -1),
				# # (overlay_set_alpha, "$reinforcement_stage_wenzi", 0xFF),
				# # (init_position,pos5),
				# # (overlay_get_position, pos5, "$reinforcement_stage"),
				# # (position_move_y, pos5, 100),
				# # (overlay_set_position, "$reinforcement_stage", pos5),
				# # (assign, "$g_reinforcement_stage", 2),
			# # (try_end),
		# # (else_try),
			# # (eq, ":object", "$reinforcement_stage"),
			# # (assign, "$g_reinforcement_stage", ":value"),
		# # (else_try),
			# # (eq, ":object", "$NPC_clash_stop"),
			# # (val_add, "$disable_npc_complaints",1),
			# # (val_mod, "$disable_npc_complaints",2),
		# # (else_try),
			# # (eq, ":object", "$cheat_box"),
			# # (val_add, "$cheat_box_val",1),
			# # (val_mod, "$cheat_box_val",2),
		# (try_end),
   # ]),
# ]),
###############
